...
void ReadSerialString(char * s,char terminator ,int maxSize){
    ...
    while (c != terminator && i < maxSize - 1){
      s[i] = c;
      //while(Serial.available() <= 0); u can use it if u have a real-time serial monitor not like the arduino ...
      c = Serial.read();
      i++;
    }
    s[i] = '\0';
...
#define GYRO_X_OFFSET -1.445798
#define GYRO_Y_OFFSET 0.663499 
#define GYRO_Z_OFFSET -0.508336 
...
#define ACCE_X_OFFSET 0.010359
#define ACCE_Y_OFFSET 0.027394
#define ACCE_Z_OFFSET -0.002585
...
double update_pitch(){
    ...
    pitch = atan(-acce_y / (acce_z * acce_z + acce_x * acce_x)) * 180 / PI - INTIAL_PITCH;
    system_pitch = elevation - pitch;
    return pitch;
...
#define ERROR_AZIMUTH DEGREE_STEP
void stepper_move(){
    double full_pos = yaw;
...
void loop(){
    ...
    if(millis() - servo_time_ms > SERVO_TIME_FEEDBACK_MS && FLAG_ELEVATION_CHANGE){
        servo_move();
    }
