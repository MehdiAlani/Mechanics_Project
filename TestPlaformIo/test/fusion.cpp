...
double pitch,pitch_gyro,pitch_acce,yaw;
MPU6050 mpu;
    ...
    while(integrate == true){
        ...
        if(micros() - startTime_us >= MIN_STEP_TIME_US){
            ...
            */
            ...
            yaw_sum += gyro_z;
            if(true) yaw -= (gyro_z / 1000) * ((micros() - startTime_us) / 1000.0);
            pitch = (atan(acce_z / acce_y)) * 180 / PI - 90;
            if(pitch > 180) pitch = pitch - 180;
            else if(pitch < 0) pitch = 180 + pitch; 
            if(yaw >= 360) yaw = yaw - 360;
            else if(yaw < 0) yaw = 360 + yaw;
            count++;
            startTime_us = micros();
        }
        if(millis() - startTime_ms >= PRINT_TIME_MS){
